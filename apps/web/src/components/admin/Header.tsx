...
export function Header({ user, organization }: Props) {
  ...
  return (
    <header className="sticky top-0 z-40 h-16 bg-navy-900 border-b border-navy-800">
      <div className="flex items-center justify-between h-full px-6">
        {/* Mobile menu button */}
        <button className="lg:hidden p-2 text-navy-400 hover:text-white">
          <Menu className="w-6 h-6" />
        </button>

        {/* Organization name */}
        <div className="hidden lg:block">
          <h1 className="text-lg font-medium text-white">{organization.name}</h1>
        </div>
